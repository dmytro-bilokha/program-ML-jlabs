...
public class NeuralNetwork {
    ...
    public static void main(String[] cliArgs) {
        ...
        var reportingQueue = new LinkedBlockingQueue<MatrixPair>();
        var reporter = new Reporter(xTrain, yTrain, xTest, yTest, reportingQueue);
        reporter.start();
        train(xTrain, yTrain, reportingQueue, 200, 10000, 0.01d);
        reportingQueue.add(new MatrixPair(null, null));
    }
    ...
    private static MatrixPair train(
            DoubleMatrix xTrain,
            DoubleMatrix yTrain,
            Queue<MatrixPair> reportingQueue,
            int hiddenNodes,
            int iterations,
            double lr) {
        ...
        for (int i = 0; i < iterations; i++) {
            reportingQueue.add(new MatrixPair(w1, w2));
            MatrixPair yHatH = calculateForward(xTrain, w1, w2);
            var yHat = yHatH.first();
            var h = yHatH.second();
            MatrixPair gradients = calculateBack(xTrain, yTrain, yHat, w2, h);
    ...
    static class Reporter extends Thread {
        ...
        private final BlockingQueue<MatrixPair> reportingQueue;

        Reporter(DoubleMatrix xTrain, DoubleMatrix yTrain, DoubleMatrix xTest, int[] yTest, BlockingQueue<MatrixPair> reportingQueue) {
            this.xTrain = xTrain;
            this.yTrain = yTrain;
            this.xTest = xTest;
            this.yTest = yTest;
        ...
        public void run() {
            ...
            try {
                while ((matrixPair = reportingQueue.take()).first() != null) {
                    report(matrixPair.first(), matrixPair.second());
                }
        ...
        private void report(DoubleMatrix w1, DoubleMatrix w2) {
